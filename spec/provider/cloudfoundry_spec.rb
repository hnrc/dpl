...
describe DPL::Provider::CloudFoundry do
  ...
  describe "#check_auth" do
    example do
      expect(provider.context).to receive(:shell).with('[[ $TRAVIS_OS_NAME = "linux" ]] && rel="linux64-binary" || rel="macosx64"; wget \'https://cli.run.pivotal.io/stable?release=${rel}&source=github\' -qO cf.tgz && tar -zxvf cf.tgz && rm cf.tgz')
